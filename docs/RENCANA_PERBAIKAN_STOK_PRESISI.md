...
| 18 | **Beberapa jalur multi-item TIDAK atomik hari ini** — ditemukan di verifikasi ke-12, **diperluas & dikoreksi di verifikasi ke-13**: `transaction_repo.go` `Create()` (checkout kasir) menjalankan loop item pakai `Exec`/`Raw` biasa TANPA transaksi DB. Dokumen sempat salah sebut `Void` sebagai "sudah benar" untuk pembanding — **setelah dicek ulang baris per baris, `transaction_repo.go` `Void()` DAN `purchase_repo.go` `Void()` ternyata SAMA-SAMA kena bug ini** (bukan cuma `Create`). Total 3 fungsi kena: `transaction_repo.go` `Create`, `transaction_repo.go` `Void`, `purchase_repo.go` `Void`. Kalau ada item ke-N gagal di tengah loop, item sebelumnya sudah permanen ter-commit — sukses sebagian diam-diam. Kabar baik: `expiry_batch_repo.go` `WriteOff` ternyata **contoh yang benar-benar sudah benar** (transaksi + `FOR UPDATE` locking) — jadi pola yang dibutuhkan Fase 2 bukan hal baru di kodebase ini, tinggal dicontoh dari situ, bukan dirancang dari nol. | Bungkus seluruh loop di **ketiga fungsi** (`transaction_repo.go` `Create` & `Void`, `purchase_repo.go` `Void`) dalam `r.db.Transaction(func(tx *gorm.DB) error {...})`, contoh polanya dari `expiry_batch_repo.go` `WriteOff` (transaksi + `FOR UPDATE`) — masuk **wajib** Fase 4 poin 1 & 2. |
...
Tahap desain sudah disepakati, sudah dipecah jadi fase pengembangan. **Belum ada implementasi kode.**
