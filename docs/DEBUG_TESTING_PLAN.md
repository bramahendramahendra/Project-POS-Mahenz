...
Jalankan Fase 3.2: debug edit dan delete Pembelian Supplier. Pastikan tombol Edit muncul untuk PO status apapun (unpaid/partial/paid) selama belum di-void, dan tombol Delete cuma muncul untuk PO yang sudah di-void. Test edit di ketiga tier pembayaran: unpaid (semua field bebas), partial (Diskon & Status Pembayaran terkunci), paid (+Metode Pembayaran ikut terkunci) — verifikasi banner info tampil sesuai tier. Test edit qty/harga/produk (verifikasi stok lama di-rollback dan stok baru benar). Coba turunkan total sampai di bawah paid_amount (harus ditolak, live di FE maupun API langsung), coba naikkan total pada PO Lunas (harus berhasil, status otomatis berubah jadi "Bayar Sebagian", paid_amount tidak berubah). Coba ganti/hapus produk yang sebagian stoknya sudah terjual di jalur lain (harus ditolak, live di FE maupun API) dan yang stoknya masih cukup (harus berhasil). Test alur Void → Delete: pastikan Delete ditolak selama PO belum di-void, lalu berhasil setelah di-void (verifikasi stok TIDAK dikurangi dua kali). Fix bug yang ditemukan, lalu type-check+lint+build sampai bersih.
```
