...
import type { CartItem, Discount, DiscountType, Tax } from '../cashier/cashier.types'
import { calcDiscountAmount, calcItemSubtotal, calcTaxAmount, calculateItemDiscount } from '../cashier/cashier.utils'

const DEFAULT_DISCOUNT: Discount = { type: 'none', value: 0, amount: 0 }
const DEFAULT_TAX: Tax = { percent: 0, amount: 0 }

interface BackdateCashierState {
  cart: CartItem[]
  discount: Discount
  tax: Tax
...
export const useBackdateCashierStore = create<BackdateCashierState>()(
  persist(
    (set, get) => ({
      cart: [],
      ...
      updateQty: (productId, unitId, qty) => {
        if (qty <= 0) { get().removeFromCart(productId, unitId); return }
        const { cart, discount, tax } = get()
        const newCart = cart.map((i) => {
          if (i.product_id !== productId || i.unit_id !== unitId) return i
          if (i.discount_type && i.discount_value) {
            const { discount_amount, effective_price, subtotal } = calculateItemDiscount(i.price, qty, i.discount_type, i.discount_value)
      ...
      updateNotes: (productId, unitId, notes) => {
        set((state) => ({ cart: state.cart.map((i) => i.product_id === productId && i.unit_id === unitId ? { ...i, notes } : i) }))
      },
      ...
      updatePrice: (productId, unitId, price) => {
        ...
        const newCart = cart.map((i) => {
          if (i.product_id !== productId || i.unit_id !== unitId) return i
          if (i.discount_type && i.discount_value) {
            const { discount_amount, effective_price, subtotal } = calculateItemDiscount(price, i.qty, i.discount_type, i.discount_value)
      ...
      setItemDiscount: (productId, unitId, type, value) => {
        ...
        const newCart = cart.map((i) => {
          if (i.product_id !== productId || i.unit_id !== unitId) return i
          if (value <= 0) {
            return { ...i, discount_type: undefined, discount_value: undefined, discount_amount: undefined, effective_price: undefined, subtotal: calcItemSubtotal(i.qty, i.price) }
          }
          const { discount_amount, effective_price, subtotal } = calculateItemDiscount(i.price, i.qty, type, value)
      ...
      clearCart: () => set({ cart: [], discount: DEFAULT_DISCOUNT, tax: DEFAULT_TAX, selectedCustomer: null }),
      ...
      setDiscount: ({ type, value }: { type: DiscountType; value: number }) => {
        ...
        set({ discount: { type, value, amount }, tax: { ...tax, amount: taxAmount } })
      },
